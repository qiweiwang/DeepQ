...
HIDDEN_LAYER = [32] #also modify initialization_parameters(), forward_prop()
...
arriveLogName = 'arrive'+suffix
emptyLogName = 'empty'+suffix
serveLogName = 'serve'+suffix
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        ...
        fd_loss = open(lossLogName,'w')
        fd_reward = open(rewardLogName,'w')
        fd_dropped = open(dropLogName,'w')
        fd_arrive = open(arriveLogName,'w')
        fd_empty = open(emptyLogName,'w')
        fd_serve = open(serveLogName,'w')

        print('Length of Horizon is {}'.format(HORIZON))
        print('Batch_size is {}'.format(BATCH_SIZE))
        print('Number of Epoch is {}'.format(NUM_EPOCH))
        print('Hidden Layers:')
    ...
    ### learning phase
        ...
        for reset in range(NUM_EPOCH):
            ...
            for writeCount in range(USERCASE):
                fd_dropped.write(str(ev.packetDroped[writeCount])+'\n')
                fd_arrive.write(str(ev.packetArrived[writeCount])+'\n')
                fd_empty.write(str(ev.emptySent[writeCount])+'\n')
                fd_serve.write(str(ev.served[writeCount])+'\n')
        fd_reward.close()
        fd_loss.close()
        fd_dropped.close()
        fd_arrive.close()
